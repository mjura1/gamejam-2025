...
func _process(_delta: float) -> bool:
	if not forced_tutorial_seen:
		...
		# instead of a real tier map (see GameFlow._initialize_game()) - that
		# map's floor layout doesn't match this test's tier assumptions at
		# all. Force it past that gate the same way smoke_legacy_shop_flow.gd
		# forces final_boss_beaten - must run here (frame 1), not
		# _initialize(), since MetaProgress.load_progress() in _ready() runs
		# after _initialize() and would clobber it.
