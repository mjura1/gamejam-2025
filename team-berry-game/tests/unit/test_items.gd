...
func test_try_buy_item_spends_upgrade_items_and_grants_item():
	var pm = PlayerManagerScript.new()
	var cost := ItemData.get_buy_cost("extra_move")
	pm.add_upgrade_items(cost + 1)
	assert_true(pm.try_buy_item("extra_move"), "buy should succeed with enough upgrade items")
	assert_eq(pm.upgrade_items, 1, "buy should spend exactly buy_cost")
...
func test_try_sell_item_refunds_and_removes():
	...
	assert_eq(pm.upgrade_items, ItemData.get_sell_value("extra_move"), "sell should refund sell_value")
...
func test_try_sell_reserve_piece_refunds_and_removes():
	...
	assert_eq(pm.upgrade_items, ItemData.get_piece_sell_value("friendly_rook"), "selling a reserve piece should refund its sell value")
...
func test_try_sell_active_piece_refunds_and_removes_when_not_last():
	...
	assert_eq(pm.upgrade_items, ItemData.get_piece_sell_value("friendly_rook"), "selling an active piece should refund its sell value")
