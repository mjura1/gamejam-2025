...
- **Luck**: `PlayerManager.get_luck()`, now `base_luck` (permanent, from
  meta-progression unlocks below) plus a run-scoped sum of owned items'
  `luck_bonus`. Higher luck grants more reroll attempts (best-of-N,
  advantage-style) on rarity and more bonus items per chest, both capped and
  tunable via `treasure_config.json`.
...
# Winter March — Permanent meta-progression: Legacy Points + shop → features/meta-progression (merged to develop)
